...
export default function LearningCentre({ slug, onBack }) {
  ...
  if (article) return <main className="learning-centre-page"><article className="learning-article"><button className="learning-back" onClick={onBack}>Back to Learning Centre</button><p className="learning-kicker">{article.category || 'Parenting guides'}</p><h1>{article.title}</h1><p className="learning-byline">By {article.author_name || 'SnuggleUp Baby Store'}{article.published_at ? ` | ${displayDate(article.published_at)}` : ''}</p><p className="learning-excerpt">{article.excerpt}</p><div className="learning-body" dangerouslySetInnerHTML={{ __html: article.body_html }} />{Array.isArray(article.product_links) && article.product_links.length > 0 && <aside className="learning-products"><h2>Explore helpful essentials</h2>{article.product_links.map((product) => <a key={`${product.source}-${product.id}`} href={product.url}>{product.name}</a>)}</aside>}</article></main>;
